...
public class PlayBoard
{
	...
	public void printBoard()
	{
		for(int i = 0; i<10; i++)
		{
			for(int j = 0; j<10;j++)
			{
				if(myBoard.get(i).get(j).getChosen() == true && myBoard.get(i).get(j).getHit() == true)
				{
					myBoard.get(i).get(j).printHit();
				}
				else if(myBoard.get(i).get(j).getChosen() == true)
				{
	...
	public void setHitBoard(int x, int y)
	{
		...
		myBoard.get(y).get(x).setChosen(true);
	}
	public void setChosenBoard(int x, int y)
	{
		myBoard.get(y).get(x).setChosen(true);
	}
}
